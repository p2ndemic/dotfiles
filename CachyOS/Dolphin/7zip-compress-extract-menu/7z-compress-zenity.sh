...
# Полная версия скрипта архивации с управлением прерыванием и поддержкой горячих клавиш
...
if [ ${#files[@]} -eq 1 ]; then
    ...
    # Вывод окна zenity для ввода имени архива
    archive_name=$(zenity --entry --title="Archive Name" --text="Enter archive name" --entry-text="$archive_name")
...
case "$action" in
    ...
    "-pack7zMax")
        ...
        ;;
    "-pack7zPass")
        password=$(zenity --title="Password protection" --entry --text="Enter archive password")
        if [ -n "$password" ]; then
...
notify-send --app-name="🐬 Dolphin" \
    ...
    "✅ Success" \
    "Archive created: <a href='file://$archive_full_name'>$archive_name$extension</a>"
